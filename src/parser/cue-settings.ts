 ...
 * @param {import('./model.ts').Cue} cue
 * @param {import('./model.ts').Region[]} regions - List of known regions for lookup
 */
export function parseCueSettings(input: string, cue: Cue, regions: Region[]): void {
      ...
      switch (name) {
          ...
          // value can be: number, number%, or number,align or number%,align
          {
              ...
              if (alignVal) {
                  if (['start', 'center', 'end'].includes(alignVal)) {
                      cue.lineAlign = alignVal as "start" | "center" | "end";
                  }
              }
        ...
        case 'position':
            {
                ...
                if (alignVal) {
                    if (['line-left', 'center', 'line-right'].includes(alignVal)) {
                        cue.positionAlign = alignVal as "line-left" | "center" | "line-right";
                    }
                }
            }
            break;
        ...
        case 'align':
            if (['start', 'center', 'end', 'left', 'right'].includes(value)) {
                cue.align = value as "start" | "center" | "end" | "left" | "right";
            }
            break;
      }
    }
